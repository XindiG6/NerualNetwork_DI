...
np.random.seed(500)  # 1 50 200 500 1000 (102)
step_size = 0.01 #
batch_size = 16 # little always better
max_epochs = 400 # 200
...
"""test function """
def test(model, X,valacc):
  # model：net
  # X:input data
  # valacc: for filename
  print("----------------------test begin------------------")
  import csv
  f = open(str(valacc)+"_val.csv", 'w', encoding='utf-8',newline='')
  csv_writer = csv.writer(f)
  csv_writer.writerow(["id", "digit"])
  ...
  while j < X.shape[0]:
    ...
    logits = model.forward(X_batch)
    tmp = logits[0].tolist()
    prediction = tmp.index(max(tmp))  # prediction val
    csv_writer.writerow([str(j), str(prediction)])
    j += 1
  f.close()
  print("----------------------test end------------------")
